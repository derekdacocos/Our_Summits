...
import { formatDate, formatElevation } from "@/lib/data";
import type { Hike } from "@/lib/types";

export function HikeCard({ hike }: { hike: Hike }) {
  return (
    <article className="flex h-full flex-col overflow-hidden rounded-lg border border-forest/10 bg-white shadow-soft">
      ...
      <div className="flex flex-1 flex-col p-5">
        <div className="mb-3 flex flex-wrap gap-2 text-xs text-ink/60">
          ...
          <span className="inline-flex items-center gap-1"><Mountain size={14} />{formatElevation(hike)}</span>
        </div>
        ...
        <p className="mb-5 mt-4 text-sm leading-6 text-ink/70">{hike.memory}</p>
        <Link href={`/summits/${hike.slug}`} className="mt-auto inline-flex w-fit rounded-full bg-forest px-5 py-2 text-sm font-semibold text-paper transition hover:bg-ember">
          View Memory
        </Link>
      </div>
    </article>
